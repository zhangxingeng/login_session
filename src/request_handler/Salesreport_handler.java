	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(report.equals("totalearning")) {
				...
				String q_earn_per_type= "SELECT brand, model ,SUM(curr.max)*0.04 FROM (SELECT brand, model, MAX(b.price) max FROM item i, bids b WHERE i.brand = b.brand, i.mpodel = b.model GROUP BY item_num) AS curr GROUP BY(curr.brand,curr.model) SORT BY i.brand, i.model";         ;
